...
def draw(canvas):
    ...
    if (ball_pos[0] <= PAD_WIDTH + BALL_RADIUS) and (paddle1_pos <= ball_pos[1] <= paddle1_pos + PAD_HEIGHT):
        ball_vel[0] = - ball_vel[0]
    if (ball_pos[0] >= WIDTH - (PAD_WIDTH + BALL_RADIUS)) and (paddle2_pos <= ball_pos[1] <= paddle2_pos + PAD_HEIGHT):
        ball_vel[0] = - ball_vel[0]
     
    # draw scores
    canvas.draw_text(str(score1), [120,30], 30, "Blue")
    canvas.draw_text(str(score2), [420,30], 30, "Red") 
